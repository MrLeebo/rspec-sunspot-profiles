...
RSpec.describe "Sunspot profile cache benchmark shape" do
  TeachingTaxonomy::EXECUTABLE_BENCHMARK_PROFILES.each_with_index do |profile_name, index|
    TeachingTaxonomy::EXECUTABLE_BENCHMARK_REPEAT_COUNT.times do |repeat_index|
      it "runs executable benchmark profile #{index + 1} (pass #{repeat_index + 1})", sunspot_profile: profile_name do
  ...
  TeachingTaxonomy::STATIC_BENCHMARK_PROFILES.each_with_index do |profile_name, index|
    TeachingTaxonomy::STATIC_BENCHMARK_REPEAT_COUNT.times do |repeat_index|
      it "runs cacheable benchmark profile #{index + 1} (pass #{repeat_index + 1})", sunspot_profile: profile_name do
        expect(sunspot_profile_names).to include(profile_name.to_s)
